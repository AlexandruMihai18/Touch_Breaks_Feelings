...
def plot_metric_bars(stats: pd.DataFrame, metric: str, output: Path) -> None:
    plot_style.apply()
    ...
    for i, (_, row) in enumerate(stats.iterrows()):
        v = row[metric]
        if not np.isnan(v):
            ax.text(i, v + 0.025, f"{v:.2f}",
                    ha="center", va="bottom", fontsize=8, fontweight="semibold",
                    color=plot_style.DARK)

    mean_val = np.nanmean(vals)
    ax.axhline(mean_val, color=plot_style.GRAY, linestyle="--", linewidth=1.0,
    ...
    ax.set_title(f"{metric.capitalize()} by object cluster")
...
def plot_confusion_bars(stats: pd.DataFrame, output: Path) -> None:
    plot_style.apply()
    ...
    bar_w = 0.55
    fig, ax = plt.subplots(figsize=(max(7.5, n * 1.3), 4.2))
    ax.bar(x, tp_p,                             width=bar_w, label="TP", color=plot_style.C_TP, zorder=3)
    ax.bar(x, tn_p, bottom=tp_p,                width=bar_w, label="TN", color=plot_style.C_TN, zorder=3)
    ax.bar(x, fp_p, bottom=tp_p + tn_p,         width=bar_w, label="FP", color=plot_style.C_FP, zorder=3)
    ax.bar(x, fn_p, bottom=tp_p + tn_p + fp_p,  width=bar_w, label="FN", color=plot_style.C_FN, zorder=3)

    ax.set_xticks(x)
    ax.set_xticklabels(clusters, rotation=30, ha="right", fontsize=9)
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Proportion")
    ax.set_title("Prediction breakdown by cluster")
    ax.legend(
...
def main() -> None:
    ...
    confusion_out = base_output.parent / f"{base_output.stem}_confusion{base_output.suffix}"
    ...
    for metric in metrics:
        ...
        plot_metric_bars(stats, metric, out)
        print(f"Saved → {out}")

    plot_confusion_bars(stats, confusion_out)
